...
float g_LightPosition0[] = { 5.0f, 5.0f, 0.0f, 0.0f };
float g_LightAmbientColor0[] = { 0.2f, 0.2f, 0.2f, 0.5f };
float g_AmbientColors[][4] = {
	{1.00, 0.00, 0.00, 0.00},
	{1.00, 0.65, 0.00, 0.00},
	{0.00, 0.50, 0.00, 0.00},
	{0.00, 1.00, 1.00, 0.00},
...
void display() {
	...
	for (int i = 0; i < g_NumTeapots; i++) {
		const float thetaDegree = deltaTheta * i + g_RotationDegree; // �e�B�[�|�b�g�̈ʒu�����߂邽�߂̊p�x
		...
		// �e�B�[�|�b�g�̍��������̒l
		const float yPos = g_HeightOffset + abs(sin(M_PI * (g_RotationDegree * g_ChangeHeightSpeed + g_Teapots[i].angle * g_DefaultHeightVariation) / 180.0)); // �����̒l���������ω������邱�ƂŃe�B�[�|�b�g���㉺�Ɉړ�����
...
void timer(int val) {
	...
	g_LightPosition0[0] = cos(M_PI * g_RotationDegree * 5.0 / 180.0);
	g_LightPosition0[1] = sin(M_PI * g_RotationDegree * 5.0 / 180.0);
	glLightfv(GL_LIGHT0, GL_POSITION, g_LightPosition0);

	if((int)(g_RotationDegree / g_DeltaRotationDegree) % 50 == 0) {
		for(int i = 0; i < 4; i++) g_LightAmbientColor0[i] = g_AmbientColors[(int)(g_RotationDegree / g_DeltaRotationDegree) % 6][i];
		glLightfv(GL_LIGHT0, GL_AMBIENT, g_LightAmbientColor0);
	}
